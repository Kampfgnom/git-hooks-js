...
module.exports = {
    ...
    install: function (workingDirectory) {
        getClosestGitPath(workingDirectory, function (err, gitPath) {
            if (err) {
                throw err;
            }
 ...
 *
 * @param {String} [currentPath] cwd for the git command
 * @param {Function} callback
 * @returns {ChildProcess}
 */
function getClosestGitPath(currentPath, cb) {
    return exec('git rev-parse --git-dir', {cwd: currentPath}, function (err, stdout) {
        if (err) {
            cb(err);
            return;
        }
